...
<policies>
    <inbound>
        <cors allow-credentials="false">
            <allowed-origins>
                <origin>https://train-mdixai.mccain.com</origin>
        ...
        <validate-azure-ad-token header-name="Authorization" failed-validation-httpcode="401" failed-validation-error-message="Unauthorized" tenant-id="59fa7797-abec-4505-81e6-8ce092642190">
            <client-application-ids>
                <application-id>0bf1b480-77a4-48d0-8eed-939ae97a0d0f</application-id>
...
###################################################################################################
resource "azurerm_api_management_api" "MF_MDI_CC_API-mdixaiddds" {
  ...
  path                = "dds"
  protocols           = ["https"]
  service_url         = "https://${azurerm_container_app.MF_MDI_CC-CAPP-DDDS.ingress[0].fqdn}"

  subscription_required = false #change this back to true later

  import {
    content_format = "openapi+json-link"
    content_value  = "https://${azurerm_container_app.MF_MDI_CC-CAPP-DDDS.ingress[0].fqdn}/swagger/v1/swagger.json"
  }
...
resource "azurerm_api_management_api_policy" "API_DDDS_POLICY" {
  ...
  api_name            = azurerm_api_management_api.MF_MDI_CC_API-mdixaiddds.name
  api_management_name = azurerm_api_management.MF-MDI-CC-APIMGT.name
  resource_group_name = azurerm_resource_group.MF_MDI_CC-RG.name
...
resource "azurerm_api_management_api" "MF_MDI_CC_DEV_API-mdixaicid" {
  ...
  path                = "cid"
  protocols           = ["https"]
  service_url         = "https://${azurerm_container_app.MF_MDI_CC-CAPP-cid.ingress[0].fqdn}"

  subscription_required = false #change this back to true later

  import {
    content_format = "openapi+json-link"
    content_value  = "https://${azurerm_container_app.MF_MDI_CC-CAPP-cid.ingress[0].fqdn}/swagger/v1/swagger.json"
  }
...
resource "azurerm_api_management_api" "MF_MDI_CC_DEV_API-mdixaidcl" {
  ...
  path                = "dcl"
  protocols           = ["https"]
  service_url         = "https://${azurerm_container_app.MF_MDI_CC-CAPP-dcl.ingress[0].fqdn}"

  subscription_required = false #change this back to true later

  import {
    content_format = "openapi+json-link"
    content_value  = "https://${azurerm_container_app.MF_MDI_CC-CAPP-dcl.ingress[0].fqdn}/swagger/v1/swagger.json"
  }
